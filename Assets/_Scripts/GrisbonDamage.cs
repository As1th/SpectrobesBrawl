...
public class GrisbonDamage : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (transform.root.gameObject.GetComponent<GrisbonController>().isAttacking)
        {
            ...
            if (other.gameObject.layer == 8 && other.gameObject.transform.root.GetComponent<SpectrobeController>().iframe == false && !hitOnce)
            {
                ...
                other.gameObject.transform.root.GetComponent<SpectrobeController>().Hit(new Vector3(hitDir.x, 4, hitDir.z) * Time.deltaTime, 700, dmg:10);
