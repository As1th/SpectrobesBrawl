...
public class BlovantDamage : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (transform.root.gameObject.GetComponent<BlovantController>().isAttacking)
        {
            ...
            if (other.gameObject.layer == 8 && other.gameObject.transform.root.GetComponent<SpectrobeController>().iframe == false && !hitOnce)
            {
                ...
                other.gameObject.transform.root.GetComponent<SpectrobeController>().Hit(new Vector3(hitDir.x, 1, hitDir.z) * Time.deltaTime, 650, dmg:10);
